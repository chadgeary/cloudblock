...
runcmd:
  - [ bash, -c, "DNS_SERVER=$(resolvectl | awk -F ': ' '/DNS Servers/ { print $2 }') && DNS_SEARCH=$(grep '^search ' /etc/resolv.conf) ; systemctl disable systemd-resolved ; systemctl stop systemd-resolved ; rm -f /etc/resolv.conf ; echo nameserver $DNS_SERVER > /etc/resolv.conf ; echo options edns0 >> /etc/resolv.conf" ]
