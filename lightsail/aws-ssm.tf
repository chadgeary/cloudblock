# lightsail activation
resource "aws_ssm_activation" "ph-ssm-activation" {
  name                    = "${var.name_prefix}-ssm-activation"
  description             = "Lightsail activation"
  iam_role                = aws_iam_role.ph-instance-iam-role.name
  registration_limit      = "1"
  tags                    = {
    Name                    = "${var.name_prefix}-instance"
    cloudblock              = random_string.ph-random.result
  }
  depends_on              = [aws_iam_role_policy_attachment.ph-iam-attach-ssm, aws_iam_role_policy_attachment.ph-iam-attach-ssmdir, aws_iam_role_policy_attachment.ph-iam-attach-ssmparameter, aws_iam_role_policy_attachment.ph-iam-attach-s3]
}
...
resource "aws_ssm_association" "ph-ssm-assoc" {
  ...
  targets {
    key                   = "tag:cloudblock"
    values                = [random_string.ph-random.result]
  }
  output_location {
