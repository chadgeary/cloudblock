# /etc Note
A special version of cloudblock/oci, but deploys two instances instead of one - within the free tier limits. The two instances share the cloud network (among other things), but are distinct/separate wireguard/pihole services.
